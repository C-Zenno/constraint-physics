# /pre-push-check — Constraint-Physics Push Gate
...
Pattern: `^public-cphy-v[0-9]+(\.[0-9]+){1,2}$`
...
If this returns a tag matching `public-cphy-vX.Y.Z`, push is safe.
...
- `public-cphy-v0.1` → `public-cphy-v0.2`

Tag must be created **after** the commit you're pushing (tag must match HEAD).

Create the tag, then push both commit and tag explicitly:

```bash
git tag public-cphy-v<NEXT>
git push origin master
git push origin public-cphy-v<NEXT>
```
